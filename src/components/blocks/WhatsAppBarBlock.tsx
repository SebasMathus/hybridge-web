...
export const WhatsAppBarBlock = ({ block }: Props) => {
  const text = block.text || 'Quiero más información'
  ...
  return (
    <>
      <style>{`
        ...
        .hb-wa-bar__iconShake {
          display: flex;
          align-items: center;
          justify-content: center;
          transform-origin: 50% 60%;
          animation: hb-wa-shake 2.4s ease-in-out infinite;
        }
        ...
        .hb-wa-bar__notifDot {
          ...
          animation: hb-wa-badge 2.4s ease-in-out infinite;
        }

        /* Vibración suave en dos ráfagas; el punto rojo visible solo mientras “vibra” */
        @keyframes hb-wa-shake {
      ...
      <Link
        ...
        data-track-id={block.trackId || 'whatsapp-bar'}
        className="hb-wa-bar"
        style={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <span className="hb-wa-bar__iconSlot" aria-hidden>
          <span className="hb-wa-bar__iconShake">
            <svg width="36" height="36" viewBox="0 0 24 24" fill="#fff" style={{ display: 'block' }}>
          ...
          <span className="hb-wa-bar__notifDot" />
        </span>
        {text}
      </Link>
    </>
