...
export const HeroBannerBlock = ({ block }: Props) => {
    ...
    <section style={{ position: 'relative', width: '100%', height: '70vh', minHeight: '400px', maxHeight: '650px', display: 'flex', alignItems: 'center' }}>
      ...
      <div className="container-hb" style={{ position: 'relative', zIndex: 2 }}>
        <div style={{ maxWidth: '600px' }}>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.8rem, 4.5vw, 3rem)', fontWeight: 800, lineHeight: 1.15, color: '#fff', marginBottom: '16px' }}>{block.heading}</h1>
          {block.subheading && <p style={{ fontSize: '1.05rem', color: 'rgba(255,255,255,0.85)', lineHeight: 1.6, marginBottom: '28px', maxWidth: '500px' }}>{block.subheading}</p>}
          {block.ctaUrl && <Link href={block.ctaUrl} data-track-id={block.ctaTrackId || ''} style={btnStyles.primary}>{block.ctaLabel || 'Inscríbete ya'}</Link>}
        </div>
      </div>
    </section>
  )
