...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: 'Aviso de privacidad | Hybridge Education',
    description: 'Aviso de privacidad de Hybridge Education.',
  }
}
...
export default async function AvisoDePrivacidadPage({ params }: Props) {
  ...
  const title = String(legal?.avisoDePrivacidad?.title ?? 'Aviso de privacidad')
  const markdown = String(legal?.avisoDePrivacidad?.markdown ?? '')
